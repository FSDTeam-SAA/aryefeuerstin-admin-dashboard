...
//   const { data } = useQuery<PlansApiResponse>({
//     queryKey: ["plans"],
//     queryFn: async () => {
...
//                   Status
//                 </TableHead>
//                 <TableHead className="text-right font-medium px-7 text-gray-700">
//                   Action
//                 </TableHead>
//               </TableRow>
//             </TableHeader>
...
const SubscriptionManagement: React.FC = () => {
  ...
  const { data, isLoading, isError } = useQuery<PlansApiResponse>({
    queryKey: ["plans", currentPage],
    queryFn: async () => {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/plan?page=${currentPage}&limit=${RESULTS_PER_PAGE}`,
        {
  ...
  if (isError) return <p className="p-6 text-red-500">Failed to load plans</p>;
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              ...
              onClick={() =>
                setCurrentPage((p) =>
                  Math.min(pagination?.pages ?? p, p + 1)
                )
              }
            >
              <ChevronRight className="h-4 w-4" />
