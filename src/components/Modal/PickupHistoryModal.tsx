...
export function PickupHistoryModal({ pickupId }: PickupHistoryModalProps) {
  ...
  const email = customer?.email ?? pickup?.email ?? "N/A";
  const phone = customer?.phone ?? pickup?.phone ?? "N/A";
  ...
  const instructions =
    customer?.pickupInstructions ?? pickup?.pickupInstructions;

  const stores = pickup?.stores ?? [];
  const packages =
    pickup?.packages ??
    stores.flatMap((s: any) => s.packages ?? []);

  const totalPackages =
    pickup?.numberOfPackages ??
    stores.reduce((sum: number, s: any) => sum + (s.numberOfPackages ?? 0), 0);
        ...
        {isLoading ? (
          ...
          <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-2">
            ...
            <Section title="Customer Information">
              ...
              <InfoRow icon={<MapPin className="h-4 w-4" />} label="Pickup Address" value={pickupAddress} />
              {instructions && (
            ...
            {/* ================= STORES ================= */}
            {stores.length > 0 && (
              <Section title="Return Stores">
                {stores.map((s: any, i: number) => (
                  <p key={i} className="text-sm text-gray-700">
                    {s.store} — {s.numberOfPackages} package(s)
                  </p>
                ))}
              </Section>
            )}
            ...
            {/* ================= PACKAGES ================= */}
            {packages.length > 0 && (
              <Section title={`Packages (${totalPackages})`}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  {packages.map((pkg: any, i: number) => (
                    <div key={i} className="border rounded-md p-3 text-sm">
                      <p className="font-medium">
                        Package No: {pkg.packageNumber ?? "N/A"}
                      </p>
                      <div className="flex gap-2 mt-2 flex-wrap">
                        {pkg.barcodeImages?.length > 0 ? (
                          pkg.barcodeImages.map((img: string, idx: number) => (
                            <Image
                              key={idx}
                              src={img}
                              alt="barcode"
            ...
            {/* ================= OPTIONS ================= */}
            {(pickup.options || pickup.needShippingLabel) && (
              <Section title="Options">
                <p className="text-sm">Shipping Label: {pickup.needShippingLabel ? "Yes" : "No"}</p>
                <p className="text-sm">Receipt: {pickup.hasReceipt ? `Yes (****${pickup.creditCardLast4})` : "No"}</p>
                {pickup.options?.message?.note && (
                  <p className="text-sm text-gray-600">
